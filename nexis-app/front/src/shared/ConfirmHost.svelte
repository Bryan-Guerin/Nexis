...
{#if $confirmState}
  <div class="backdrop" style="z-index:1500" onclick={() => resolveConfirm(false)}>
    <div class="modal confirm-modal" onclick={e => e.stopPropagation()}
      ...
      {#if $confirmState.message}<p class="confirm-msg" id="confirm-msg">{$confirmState.message}</p>{/if}
      <div class="modal-actions">
        ...
        <button class:btn-danger={$confirmState.danger} class:btn-primary={!$confirmState.danger}
                onclick={() => resolveConfirm(true)} autofocus>{$confirmState.confirmLabel}</button>
      </div>
    </div>
  </div>
{/if}
